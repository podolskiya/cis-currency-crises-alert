...
from src.config import (PANEL_START, COUNTRY_START_OVERRIDE,
                        EXCLUDE_FROM_MODEL, POLICY_REFORM_EVENTS)
...
p["fx_ret"] = np.where(p["COUNTRY"].eq("UZB"), p["dlog_pa"], p["dlog_eop"])
p["res_ret"] = p["dlog_res"]

sd = p.groupby("COUNTRY")[["fx_ret", "res_ret"]].std()
w = (1 / sd).div((1 / sd).sum(axis=1), axis=0).rename(
    columns={"fx_ret": "w_fx", "res_ret": "w_res"})
...
p["crisis_raw"] = (p["emp_z"] > THRESHOLD_SD) & (p["fx_ret"] > MIN_FX_MOVE)
